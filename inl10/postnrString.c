...
area *binary_search(codes *data, const char *zip) {
    int left = 0, right = data->n - 1;

    while (left <= right) {
        int mid = left + (right - left) / 2;
        int cmp = strcmp(data->areas[mid].zip, zip);
...
long bench_linear(codes *data, const char *zip, int loop, int itr) {
...
long bench_binary(codes *data, const char *zip, int loop, int itr) {
    struct timespec t_start, t_stop;
    long min = LONG_MAX;
    for (int i = 0; i < loop; i++) {
        long time = LONG_MAX;
